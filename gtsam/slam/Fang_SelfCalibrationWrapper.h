...
class SelfCalibrationWrapper {
  ...
  protected:
    ...
    size_t fixed_pose_id_;
    std::shared_ptr<Pose3> ptr_fixed_pose_;
  ...
  public:
    ...
    {
      fixed_pose_id_ = id;
      ptr_fixed_pose_ = std::make_shared<Pose3>(pose);
    }


    /**@brief pose prior: to remove the global 6DOF gauge/transformation ambiguitiey */
                         ...
                         double sigma_rot = 0.001 /*rad on roll, pitch, yaw*/, double sigma_tran = 0.003 /*m on x, y, z*/)
    {
      ...
      graph_.addPrior(gtsam::Symbol('x', ith_pose), gtsam::Pose3(pose), poseNoise);
    }
    ...
    bool add_landmark_depth_prior (size_t ith_pose, size_t jth_landmark, double depth, double sigma = 1.0)
    {
      if (depth <=0) {
      ...
      if (ith_pose == fixed_pose_id_)
      {
        graph_.emplace_shared<FixedPoseLandmark3DDepthFactor>(
            depth,
            ptr_fixed_pose_,
            depth_noise,          
            gtsam::Symbol('l', jth_landmark)
      ...
      {
        graph_.emplace_shared<Landmark3DDepthFactor>(
            depth,
            depth_noise,
            gtsam::Symbol('x', ith_pose),
            gtsam::Symbol('l', jth_landmark)
  ...
  public:
    ...
    */
    ...
    //   }
      ...
      /**
       * @brief add initial values for the optimizer 
       * */
      ...
      if (verbose_ & VERBOSE::RESULT) {
        ...
        std::cout << "Init: \t" <<graph_.error(initialEstimate)<< "\n";
        std::cout << "Opt:  \t" <<graph_.error(result)<< "\n"; 
      }
...
protected:
    ...
    void addLandmarkPrior (size_t jth_landmark, gtsam::Point3 landmark_3d, double sigma = 0.001) {
      ...
      graph_.addPrior(Symbol('l', jth_landmark), landmark_3d, pointNoise);
    }
    ...
    void addCalibrationPrior (CALIBRATION K){
        ...
        graph_.addPrior(Symbol('K', 0), K, calNoise);
    }
    ...
    void addRRelativePoseTranslationDistancePrior (size_t pth_pose, size_t qth_pose, double distance, double sigma = 0.001) {
      ...
      graph_.emplace_shared<RelativePoseTranslationDistanceFactor>(
          distance,
          dist_noise,
    ...
    void addLandmarkNormPrior (size_t jth_landmark, double distance, double sigma = 0.001) {
      auto dist_noise = noiseModel::Isotropic::Sigma(1, sigma);
      graph_.emplace_shared<Point3NormFactor>(
          distance,
          dist_noise,
          gtsam::Symbol('l', jth_landmark)
          );
    ...
    void addRelativePointDistancePrior (size_t pth_landmark, size_t qth_landmark, double distance, double sigma = 0.001) {
      auto dist_noise = noiseModel::Isotropic::Sigma(1, sigma);
      graph_.emplace_shared<RelativePointDistanceFactor>(
          distance,
          dist_noise,
...
/**
 * @brief A convenient typedef to use the calibration wrapper
 * */
namespace gtsam {
...
//     SelfCalibrationWrapperBackup() : graph_(), verbose_(false) {}
...
//       auto poseNoise = gtsam::noiseModel::Diagonal::Sigmas( (Vector(6) << gtsam::Vector3::Constant(sigma_rot), gtsam::Vector3::Constant(sigma_tran)).finished());
//       graph_.addPrior(gtsam::Symbol('x', ith_pose), pose, poseNoise);
//     }
